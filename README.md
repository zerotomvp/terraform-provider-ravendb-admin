...
* `pem_base64` is now populated when the RavenDB ZIP response contains separate `.crt` and `.key` files instead of an embedded `.pem`.
...
* Removed volatile `documents_count` and `indexes_count` fields from the `ravendb_databases` data source. They fluctuate on every read and caused spurious drift on every plan.

### v0.1.0

#### Features

* Initial release: `ravendb_database` and `ravendb_certificate` resources
* `ravendb_databases` data source (list)
